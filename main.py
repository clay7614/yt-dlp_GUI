...
class App(ctk.CTk):
    ...
    def select_language(self, language):
        ...
        global _
        _ = gettext.translation("messages", localedir="locale", languages=[language], fallback=True).gettext
        self.restart(self)
    ...
    def set_submenu_color(self, submenu, dict_map, option):
        target = [k for k, v in dict_map.items() if v == option][0]
        for c in submenu:
...
class QuickMode:
    def __init__(self):
        ...
        self.icon = Icon("yt-dlp_GUI", image, "yt-dlp_GUI", menu)
        self.icon.run()
    def quit(self):
        app.deiconify(); self.icon.stop()
    def download(self):
        app.paste(); app.start_download()

if __name__ == "__main__":
    conf = ConfigManager()
    lang = conf.get("Option", "language", "jp")
    _ = gettext.translation("messages", localedir="locale", languages=[lang], fallback=True).gettext
    app = App()
    app.protocol("WM_DELETE_WINDOW", lambda: app.write_config(False))
    app.mainloop()
